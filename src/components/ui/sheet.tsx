...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-background p-6 shadow-lift transition ease-in-out data-[state=closed]:duration-200 data-[state=open]:duration-300 data-[state=open]:animate-in data-[state=closed]:animate-out',
  {
    variants: {
      side: {
...
function SheetTitle({ className, ...props }: ComponentProps<typeof SheetPrimitive.Title>) {
  return (
    <SheetPrimitive.Title className={cn('font-display text-xl font-medium', className)} {...props} />
  )
}
